...
1. Save new export files into `data/raw/` (gitignored — never commit real financial data). CSV and `.xlsx` both parse (`common.read_logical_rows()` branches on the extension; for a multi-tab `.xlsx` the parsers pick their own tab by name). PDF does not — if someone hands you a PDF bank/card statement, either ask for a CSV/XLSX export instead, or use the PDF only as a cross-check on numbers already imported; don't build a PDF import path. Note that an `.xlsx` export is not always a like-for-like swap for the CSV: see the Cal currency gotcha in section 4.
...
- **Cal currency detection differs between CSV and .xlsx.** In a CSV, Cal writes the symbol into the amount cell (`$20.00`) and `detect_foreign_currency()` catches it per row. In the `.xlsx` the amount is a bare numeric cell and the symbol survives only on the section's subtotal line (`עסקאות בחיוב מיידי 45.00 $`), so `detect_section_currency()` supplies it as a fallback. That fallback deliberately ignores any line also containing `₪`: a real export carries a pending-summary line reading `עסקאות בתהליך קליטה 1,035.00 ₪ ובנוסף 211.29 $` which sits *above* the ordinary shekel rows, and treating it as a section marker would convert a whole file of ILS rows. Symptom of this going wrong: the main bundle's charge-date total is off by exactly the foreign rows' raw sum.
- **Cal pending rows** (`עסקה בקליטה`) sit in the same tab as real rows with an empty `סכום חיוב`, so they import as ₪0 placeholders that never self-update — and import again for real next cycle. Max's pending rows are on their own tab and are skipped automatically; Cal's are not. Drop them from the file before importing.
